...
class EventCheckoutController extends Controller
{
    ...
    public function completeOrder($event_id, $return_json = true)
    {
        ...
        try {
            ...
            $order = new Order();
            $ticket_order = session()->get('ticket_order_' . $event_id);
            // properly cancel offline registrations
            // cf https://github.com/Attendize/Attendize/issues/351
            $access_count = count($ticket_order['request_data']);
            $request_data = $ticket_order['request_data'][$access_count - 1];
            ...
            foreach ($ticket_order['tickets'] as $attendee_details) {
                ...
                for ($i = 0; $i < $attendee_details['qty']; $i++) {
                    ...
                    $attendee->email = $order->email;// $request_data["ticket_holder_email"][$i][$attendee_details['ticket']['id']];
